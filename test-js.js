...
<section id="fvtbook" class="py-20 px-6 md:px-12" style="background:#0d0015">
  <div class="max-w-4xl mx-auto">
    <div class="mb-10">
      ...
      <a onclick="window.open(window.location.href.split('#')[0]+'#fvtbook','_blank')" style="cursor:pointer;color:rgba(255,140,26,.7);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;display:inline-flex;align-items:center;gap:5px;margin-bottom:16px">🔗 Otevřít komunitu ve vlastní záložce</a>
    </div>
    ...
    <div class="rounded-2xl p-8 mb-8 border border-white/10" style="background:rgba(255,255,255,.04)">
      <h3 class="text-white font-black uppercase text-lg tracking-tight mb-6">Přidat příspěvek</h3>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <input id="fvt-nick" type="text" placeholder="Přezdívka *" maxlength="40"
          class="w-full bg-white/8 border border-white/15 rounded-xl px-4 py-3 text-white text-sm placeholder-white/40 focus:outline-none focus:border-fest-orange" style="background:rgba(255,255,255,.06)"/>
        <input id="fvt-email" type="email" placeholder="E-mail (nebude zveřejněn)"
          class="w-full bg-white/8 border border-white/15 rounded-xl px-4 py-3 text-white text-sm placeholder-white/40 focus:outline-none focus:border-fest-orange" style="background:rgba(255,255,255,.06)"/>
      </div>
      <textarea id="fvt-msg" placeholder="Napiš vzkaz ostatním festivalákům…" rows="3" maxlength="280"
        class="w-full bg-white/8 border border-white/15 rounded-xl px-4 py-3 text-white text-sm placeholder-white/40 focus:outline-none focus:border-fest-orange mb-4 resize-none" style="background:rgba(255,255,255,.06)"></textarea>
      <!-- Photo upload -->
      <label class="flex items-center gap-3 mb-4 cursor-pointer group">
        <div class="border border-dashed border-white/25 rounded-xl px-4 py-2 text-white/50 text-xs font-bold uppercase tracking-wider group-hover:border-fest-orange transition-colors flex items-center gap-2">
          <span>📷</span> Přidat fotku (nepovinné)
        </div>
        <input id="fvt-photo" type="file" accept="image/*" class="hidden" onchange="previewPhoto(this)"/>
      </label>
      <img id="photo-preview" src="" alt="" class="hidden rounded-xl mb-4 max-h-40 object-cover"/>
      <!-- GDPR -->
      <label class="flex items-start gap-3 mb-5 cursor-pointer">
        <input id="fvt-gdpr" type="checkbox" class="mt-1 accent-orange-500 w-4 h-4 flex-shrink-0"/>
        <span class="text-white/60 text-xs leading-relaxed">Souhlasím se zpracováním osobních údajů dle <span style="color:#FF8C1A">GDPR</span>. Moje jméno a příspěvek budou zobrazeny veřejně. E-mail nebude zveřejněn ani sdílen třetím stranám.</span>
      </label>
      <button onclick="submitPost()" class="grad-btn text-white font-black uppercase tracking-widest text-sm px-8 py-3 rounded-[10px]">Přidat příspěvek →</button>
      <p id="fvt-error" class="text-red-400 text-xs mt-3 hidden"></p>
    </div>

    <!-- Message wall -->
    <div id="fvtbook-wall">
      <h3 class="text-white font-black uppercase text-lg tracking-tight mb-4">Zeď příspěvků</h3>
      <div id="fvt-posts">
        <!-- demo posts -->
        <div class="msg-card">
          <div class="flex items-center gap-2 mb-2">
            <div class="w-8 h-8 rounded-full flex items-center justify-center font-black text-xs text-white" style="background:#FF8C1A">M</div>
            <span class="font-bold text-sm text-white">Maruška_Telč</span>
            <span class="text-white/35 text-xs ml-auto">před chvílí</span>
          </div>
          <p class="text-white/80 text-sm leading-relaxed">Už se nemůžu dočkat! Rybičky 48 jsou sen 🎸🔥</p>
        </div>
        <div class="msg-card">
          <div class="flex items-center gap-2 mb-2">
            <div class="w-8 h-8 rounded-full flex items-center justify-center font-black text-xs text-white" style="background:#9B59B6">P</div>
            <span class="font-bold text-sm text-white">Pepa_FvT</span>
            <span class="text-white/35 text-xs ml-auto">2 min</span>
          </div>
          <p class="text-white/80 text-sm leading-relaxed">Třetí rok v řadě — bez Telče to prostě nejde! Kdo jde s námi kempit? ⛺</p>
        </div>
      </div>
    </div>
  </div>
...
// ─── FVT BOOK ─────────────────────────────────────────────
var AVATAR_COLORS = ['#FF8C1A','#9B59B6','#1A9E6E','#FF6600','#3498DB','#E74C3C'];
...
function submitPost() {
  var nick = document.getElementById('fvt-nick').value.trim();
  var msg  = document.getElementById('fvt-msg').value.trim();
  var gdpr = document.getElementById('fvt-gdpr').checked;
  var err  = document.getElementById('fvt-error');
  if (!nick) { err.textContent='Vyplň přezdívku.'; err.classList.remove('hidden'); return; }
  if (!msg)  { err.textContent='Napiš zprávu.'; err.classList.remove('hidden'); return; }
  if (!gdpr) { err.textContent='Je nutný souhlas s GDPR.'; err.classList.remove('hidden'); return; }
  err.classList.add('hidden');
  var photoSrc = document.getElementById('photo-preview').src;
  var color = AVATAR_COLORS[nick.charCodeAt(0) % AVATAR_COLORS.length];
  var card = document.createElement('div');
  card.className = 'msg-card';
  card.innerHTML =
    '<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">' +
    '<div style="width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:12px;color:white;background:' + color + ';flex-shrink:0">' + nick[0].toUpperCase() + '</div>' +
    '<span style="font-weight:700;font-size:14px;color:white">' + nick.replace(/</g,'&lt;') + '</span>' +
    '<span style="color:rgba(255,255,255,.3);font-size:11px;margin-left:auto">právě teď</span></div>' +
    (photoSrc && photoSrc !== location.href ? '<img src="' + photoSrc + '" style="border-radius:10px;max-height:160px;object-fit:cover;margin-bottom:8px;display:block"/>' : '') +
    '<p style="color:rgba(255,255,255,.8);font-size:14px;line-height:1.5">' + msg.replace(/</g,'&lt;') + '</p>';
  document.getElementById('fvt-posts').prepend(card);
  document.getElementById('fvt-nick').value = '';
  document.getElementById('fvt-msg').value  = '';
  document.getElementById('fvt-email').value = '';
  document.getElementById('fvt-gdpr').checked = false;
